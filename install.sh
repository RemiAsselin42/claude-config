...
# below; stock macOS ships 3.2 and would abort partway through the install.
if (( BASH_VERSINFO[0] < 4 || (BASH_VERSINFO[0] == 4 && BASH_VERSINFO[1] < 4) )); then
  echo "install.sh needs bash 4.4+ (this is $BASH_VERSION). macOS: brew install bash, then run: \$(brew --prefix)/bin/bash install.sh" >&2
  exit 1
fi
...
_generate_mempalace_yaml() {
  ...
  if [[ -f "$yaml_file" ]]; then
    ...
    if grep -q -E '^exclude:[[:space:]]*$' "$yaml_file"; then
      sed -i.bak 's/^exclude:[[:space:]]*$/exclude_patterns:/' "$yaml_file" && rm -f "$yaml_file.bak"
...
_patch_graphify_hook_nullbytes() {
  ...
  # -i.bak, not -i: BSD sed (macOS) takes the next word as the backup suffix.
  sed -i.bak 's#head -1 "$GRAPHIFY_BIN" | sed#head -1 "$GRAPHIFY_BIN" | tr -d "[:cntrl:]" | sed#' "$hook" && rm -f "$hook.bak"
}
...
# while-read, not mapfile: git runs hooks with the PATH bash, 3.2 on stock macOS.
files=()
